...
def parseSiteDump(sSites):
    ...
    while True:
        ...
        if match:
             ...
             mapSiteInfo[(sSiteID,sScheme)][iInternalCount] = [sFileName, iLine, sFunction, first, second]
             iInternalCount += 1
             continue
...
def importReportFile(sName, IDmap, iRate):
    ...
    reSampleBegin = re.compile("<samples unit=\"([0-9a-f]+)\" scheme=\"scalar-pairs\">")    
    ...
    while True:
        ...
        if match:
            ...
            iCount += int(match.group(3))
            iSmall = int(match.group(1))
            if iSmall == 0 or countDown[0] > iSmall:
                countDown[0] -= iSmall
            else:
                #ID = IDmap[(sUnit, iIndex)] * 2
                if IDmap.get((sUnit, iIndex, '<')) == None:
                    IDmap[(sUnit, iIndex, '<')] = len(IDmap)
                ID = IDmap[(sUnit, iIndex, '<')]
                doc.append(ID) 
                remaining = iSmall - countDown[0]
                del countDown[0]
                if len(countDown) == 0:
                    countDown = sampleGeometric(iRate)
            ...
            iEqual = int(match.group(2))
            if iEqual == 0 or countDown[0] > iEqual:
                countDown[0] -= iEqual
            else:
                if IDmap.get((sUnit, iIndex, '==')) == None:
                    IDmap[(sUnit, iIndex, '==')] = len(IDmap)
            ...
            iLarge = int(match.group(3))
            if iLarge == 0 or countDown[0] > iLarge:
                countDown[0] -= iLarge
            else:
                if IDmap.get((sUnit, iIndex, '>')) == None:
                    IDmap[(sUnit, iIndex, '>')] = len(IDmap)
...
if __name__ == '__main__':
    ...
    for index in range(len(rankList)):
        print mapSiteInfo[(tmpID[rankList[index][0]][0], 'scalar-pairs')][tmpID[rankList[index][0]][1]][0:3], 
        print mapSiteInfo[(tmpID[rankList[index][0]][0], 'scalar-pairs')][tmpID[rankList[index][0]][1]][3],  tmpID[rankList[index][0]][2],
        print mapSiteInfo[(tmpID[rankList[index][0]][0], 'scalar-pairs')][tmpID[rankList[index][0]][1]][4] ,  rankList[index][1]
